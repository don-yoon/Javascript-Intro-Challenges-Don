...
const stringReverse = (string1) => {
    //Function will use split method to turn the string into an array. We use """ to split every character to its own array element. Since the split() method is an accessor, we have to assign it to a new variable.
    let stringArray = string1.split("")
    //The method reverse() is a mutator so the original array will be changed
    stringArray.reverse()
    //We declare a new string to store our output from the join() method because it is an accessor
...
//Function will use a for loop to iterate each element of the array and use if else conditional statements to determine if odd or even. The function will then return a string "odd" or "even"

//First we declare our function with a parameter array1
//The function will occur outside of the for loop. The loop will pass the value of each index to the function. We will declare arrayAtIndex with var so that it can be scoped globally
...
//We start and index 0 and iterate for the length of the array 1 step at at time.
//Input is the array stored in the constant myArray ==> Output will be odd, even, odd, even, odd, even
 for (let i = 0; i < myArray.length; i++) {

        var arrayAtIndex = myArray[i]
...
const bigSubSmall = (num1, num2) => {
    //compare the numbers using conditional statements. If num1 is larger than or equal to num2, it will return num1 subtracted by num2. We used >= because if both values are the same, order of subtraction would not matter.
    if (num1 >= num2) {
        return num1 - num2
    //All other conditions will be the other way around.
    } else {
        return num2 - num1
    }
}
